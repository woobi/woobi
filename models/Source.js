...
var Source = new keystone.List('Source', {
	...
	track: true,
	drilldown: 'source program argument stream'
});
...
Source.add({
	...
	type: { type: Types.Select,  label:'type', initial:true,  options: 'file, http, program, udp',},
},	{ heading: "HTTP", dependsOn: { type: 'http' } }, {
	url: { type: Types.Url, initial:true, dependsOn: {type:'http'}  },
	...
	stream: { type: Types.Relationship, ref: 'Stream', many: false , dependsOn: {type:'program'} },
...
function pushFile(doc, file, cb) {
	...
	list.model.findOneAndUpdate({ file: file  }, send, {upsert: true, safe: true}, function(err, newdoc){
		if(err) {
