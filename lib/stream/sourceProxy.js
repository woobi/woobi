	...
	return function sourceProxy(opts, callback) {
		...
		var routes = function(app) {
			...
			app.get( ['/alvin/program/:name/:cmd', '/alvin/program/:name/:cmd/:cmd2'], function (req, res) {
				var pro = Broadcast[req.params.name];
				if(pro) {
					var save = pro.redo(cmd, cmd2);
					if(_.isFunction(pro.end)) {
						pro.end();
					}
					Broadcast[req.params.name] = new Broadcast.Source.Program(save, (err, program) => {
						res.jsonp({ success: true, options: program.options.redo });
					});
				} else {
					res.jsonp({ success: false, error: 'Program not found'});
				}
			});
