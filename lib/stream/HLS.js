 ...
 * 
 * */
var debuger = require('debug')('snowstreams:lib:stream:hls');
var debug = debuger;
...
module.exports = function(Broadcast) {
	...
	return function HLS(opts, callback) {
		...
		this.path = opts.path || path.join(__dirname, '../', '../', 'video/', 'channels', this.channel);
		
		this.urlPath = opts.urlPath || path.join('hls', 'channel',  this.channel);
		...
		if(opts.outputOptions) {
			this.program.addOptions(opts.inputOptions);
		}
		if(opts.onlyOptions) {
			...
			// set video bitrate
			this.program.videoBitrate(2500)
			// set h264 preset
		...
		this.program.on('start', (commandLine) => {
			...
			.on('error', (err) => {
				debug('an error happened in main: ' + err.message, opts.arg);
				this.crash();
			});
		if(opts.progress) {
			// progress reports
