...
module.exports = function(Broadcast) {
	...
	return function HLS(opts, callback) {
		...
		this.mediaPath = 
		this.path = opts.path || path.join(Broadcast.mediaPath, 'channels', this.id);
		
		this.urlPath = opts.urlPath || path.join('/', Broadcast.get('proxy api'), 'play', 'hls',  this.channel);
		...
		let cleanRate = (opts.cleanRate / 60);
		this.interval = setInterval(() => {
			debug('HLS clean ts files');
			try {
				findRemoveSync(this.path, {age: {seconds: cleanRate || 3600000}, extensions: '.ts'});
			} catch(e) {
				debug(e);
			}
		}, cleanRate || 3600000); //3600000
		
		//this.stream = Broadcast.Stream.bridge();	
		
		this.segmentFile = path.join(this.path, this.id + 'BB.m3u8');
		...
		this.link = 'http://' + Broadcast.host + ':' + Broadcast.port + path.join('/', this.urlPath, this.id + '.m3u8');
