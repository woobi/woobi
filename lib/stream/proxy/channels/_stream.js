...
module.exports = function _channelStreamApi(Broadcast) {
    ...
    /**
     * Send the fluent stream
     * 
     * @param {*} req 
     * @param {*} res
     * @param {*} [channel]
     */
    methods.ts = function http(req, res, channel) {
        // streams a file from anywhere on the filepath
        debug('/ts route');
        var range =  req.headers.range || undefined;
        var start;
        ...
        debug('Trying to send channel fluent stream: ', Broadcast.isReadableStream(channel.stream), start);
    ...
    methods.playlist = function playlist(req, res, channel) {
        ...
        function html() {
            ...
            links.forEach((n) => {
                ...
                if(n.currentSource && _.isObject(n.currentSource.progress)) {
                    ...
                    ret.push('<div style="clear:both" ></div>')
                    ret.push('<div style="width:50px;text-align:center;margin-right:5px;background:#1f203a;color:#fff;padding:4px;float:left;">' + n.currentSource.progress.frames + '</div>');
                    ret.push('<div style="width:50px;text-align:center;margin-right:5px;background:#1f203a;color:#fff;padding:4px;float:left;">' + n.currentSource.progress.currentFps + '</div>');
                    ret.push('<div style="width:60px;text-align:center;margin-right:5px;background:#1f203a;color:#fff;padding:4px;float:left;">' + n.currentSource.progress.currentKbps + '</div>');
                    ret.push('<div style="width:50px;text-align:center;margin-right:5px;background:#1f203a;color:#fff;padding:4px;float:left;">' + n.currentSource.progress.targetSize + '</div>');
                    ret.push('<div style="width:95px;text-align:center;margin-right:5px;background:#1f203a;color:#fff;padding:4px;float:left;">' + n.currentSource.progress.timemark + '</div>');
                    ret.push('<div style="width:60px;text-align:center;margin-right:5px;background:#1f203a;color:#fff;padding:4px;float:left;">' + Number.parseFloat(n.currentSource.progress.percent).toFixed(1) + ' %</div>');
                }
                ret.push('<div style="clear:both" ></div>')
                ret.push('</div><hr />');
            })
    ...
    _.each(Broadcast.channels, (c,k) => {
        list.push('#EXTINF:-1 tvg-name="' + c.channel + ' " group-title="' + c.channel + ' ", ' + c.channel);
        //list.push(c.links.ts);
        list.push(c.links.hls);
        if (c.options.epg.unicast === true) {
            list.push('#EXTINF:-1, tvg-name="' + c.channel + ' Unicast" group-title="' + c.channel + ' Unicast"');
        ...
        if (c.options.epg.ts === true) {
            
            list.push('#EXTINF:-1,tvg-name="' + c.channel + ' TS" group-title="' + c.channel + ' TS"');
            list.push(c.links.ts);
        }
        if (c.options.epg.http === true) {
            
            list.push('#EXTINF:-1,' + c.channel + ' http stream');
            list.push(c.links.http);
        }
    });
     ...
     * @param {*} Broadcast 
     */
    methods.m3uts = function m3u(req, res, channel) {
        //debug('hls channel playlist route');
        var list =['#EXTM3U', ''];
    ...
    methods.epg = function epg(req, res, channel) {
        ...
        var list =['<?xml version="1.0" encoding="ISO-8859-1"?>', '<!DOCTYPE tv_listings SYSTEM "xmltv.dtd">'];
        // service provider.... tv
        let today = moment().format("YYYYMMDDHHMMSS ZZ")
        list.push('<tv date="'+today+'" version="1.0"  generator-info-url="http://www.xmltv.org/">');
        var chans = new Array();
...
function serveFluentStream( fluent, req, res, Broadcast) {
    if (err || !Broadcast.isReadableStream(fluent.stream)) {
        res.end(err);
    }					
    debug('Playing ' + fluent.name );		
...
function printPgm(who, programs, item, a , b, c, s, isvod, callback) {
    ...
    programs.push('<programme start="' + a + '" stop="' + b + '" channel="' + c.channel + who + '">');
        let series = s.series || s.name || '';    
        programs.push('<title lang="en">' + vod + sanitize(series) +  '</title>');
        if(item.title) programs.push('<sub-title lang="en">' + sanitize(item.title) + '</sub-title>');
