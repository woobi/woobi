    ...
    methods.playlist = function playlist(req, res, channel) {
        ...
        function html() {
            ...
            links.forEach((n) => {
                ...
                if(!n.currentSource)ret.push('<a href="'+n.info+'">Info</a><br />');
                if(!n.currentSource) ret.push('<a href="'+n.jumpTo+'">Cycle queue to this source</a><br />');
...
function printPgm(who, programs, item, a , b, c, s, isvod, callback) {
    ...
    programs.push('<programme start="' + a + '" stop="' + b + '" channel="' + c.channel + who + '">');
        ...
        programs.push('<desc lang="en">' + desc + (sanitize(item.description) || item.plot || item.name || s.plot || 'no description available.') + '</desc>');
        if(item.movieID) {
