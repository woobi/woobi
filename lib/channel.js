...
module.exports = function(Broadcast) {
	...
	function Channel(channel, options, callback) { 
		...
		// master link
		this.link = 'http://' + Broadcast.host + ':' + Broadcast.port + path.join('/', apiLink, 'channel', this.channel);
		this.path = path.join(Broadcast._options.moduleRoot, apiLink, 'channel', this.channel);
		// other links
		this.links = {
			local: this.path,
			unicast: 'http://' + Broadcast.host + ':' + Broadcast.port + path.join('/', apiLink, 'unicast', this.channel),
			http: 'http://' + Broadcast.host + ':' + Broadcast.port + path.join('/', apiLink, 'channel', this.channel),
			hls: false,
			udpSink: [],
			udpStream: []
		};
		...
		async.forEachSeries( assets, (asset, next) => {
			...
			function done(err, hls) {
				...
				if (options.out) {
					...
					/*
					this.out = new Broadcast.Source.Fluent({
						stream: this.broadcaster,
						name: this.channel + ':out',
						output: out,
						passthrough: true,
						mpegts: options.mpegts
					}, (err, fluent) => {
						if ( err ) {
							this.debug(this.channel + ' - ' + ' Error', err);
						}
			...
			function HLS() {	
				this.debug(this.channel + ' - ' + ' Use HLS? ', options.hls == true);
				if(options.hls) {
	...
	Channel.prototype.play = function(playThis, done) {
		...
		this.debug(this.channel + ' - ' + 'Got Source?', !!source);
		if(!source) {
			if(this.loop) {
