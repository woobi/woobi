				...
				if(options.udpSink) {
					...
					async.forEach(sink, (drip, drop) => {
						this.udpSinks.push(Broadcast.Source.UDP(drip, (err, udp) => {
							...
							if(udp.addSource !== false) {
								this.debug('Add UDP Sink to Sources');
								this.addSource(udp);
							}
					 ...
					 var run = (cb, again = false) => {
						this.hls.push(new Broadcast.Stream.HLS(_.defaults(consume, { source: this.stream }), (err, Source) => {
							if(err) {
								...
								_.pullAllBy(this.end, [{ name: Source.name }], 'name');
								this.end.push({name: Source.name, end: Source.end});
