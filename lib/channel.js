...
module.exports = function(Broadcast) {
	...
	function Channel(channel, options, callback) { 
		...
		this.url = path.join('/', apiLink, 'channel', this.channel);
		// other links
		this.links = {
			local: path.join('/', apiLink, 'channel', this.channel),
			unicast: 'http://' + Broadcast.host + ':' + Broadcast.port + path.join('/', apiLink, 'channel', this.channel),
			hls: false,
			udpSink: [],
			udpStream: []
		};
		...
		async.forEachSeries( assets, (asset, next) => {
			...
			function done(err, hls) {
				...
				this.debug('Start Out?', options.out === true);
				if (options.out) {
