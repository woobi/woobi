...
module.exports = function(Broadcast) {
	function Channel(channel, options) { 
		...
		// master link
		this.link = '/alvin/channel/' + this.channel;
		// other links
		this.links = {};
		...
		this.udp = {};
		if(options.udp) {
			this.udp = new Broadcast.Stream.UDP(_.defaults(options.udp, { channel: channel, port: Broadcast.channelPort + channel }), (err, udp) => {
				if(_.isFunction(udp.play)) {
					udp.play(this.stream, (err) => {
						if(err) console.log(err);
					});
				}
				this.links.udp = 'udp://' + udp.host + ':' + udp.port;
			});
		}
		
		if(_.isObject(options.mpegts)) {
			this.mpegts = new Broadcast.Stream.MpegTS(options.mpegts, (err, Source) => {
				if(err) {
					debug('##ERROR##',err);
				} else {
					this.sources.push(Source);
					this.links.hls = Source.link;
				}
