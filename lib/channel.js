...
module.exports = function(Broadcast) {
	...
	function Channel(channel, options, callback) { 
		...
		if(!options.id) {
			...
			//console.info('##');
			this.debug('##  channel id created: ' + Broadcast.camelCaseSanitize(channel));
			//console.info('##');
		...
		this.debug(channel + ' - ' +'Channel created' );	
		...
		this.input.on('pipe', (src) => {
			//this.debug(this.id + ' - ' +' New source piped to stream');
		})
		.on('unpipe', (src) => {
			this.debug(this.id + ' - ' +' is not streaming any content');
			//this.debug(this.id + ' - ' +'### Piping QUICKFILL on unpipe ###');
			//this.quickFill.pipe(this.input);
		})
		.on('drain', (src) => {
		...
		if(Array.isArray(this.options.files)) {
			...
			this.debug('## ', this.sources.length, ' sources added to channel')
		}
		...
		this.links = {
			...
			http: 'http://' + Broadcast.host + ':' + Broadcast.port + path.join('/', this.id, 'http').replace(/\\/g,'/'),
			ts: 'http://' + Broadcast.host + ':' + Broadcast.port + path.join('/', this.id, 'ts').replace(/\\/g,'/'),
			...
			m3uts: 'http://' + Broadcast.host + ':' + Broadcast.port + path.join('/', 'm3uts').replace(/\\/g,'/'),
	...
	Channel.prototype.addSource = function(source, callback) {	
		...
		if (source.file) {
			...
			if(typeof source.metadata != 'object') source.metadata = {}
			if(!source.metadata.totalTimeInSeconds && !source.skipCheck) {
				ffprobe( source.file, ( err, metadata ) => {
					...
					if(metadata) {
						...
						source.metadata.duration = metadata.format.duration / 60;
						...
						source.metadata.totalTimeInSeconds = metadata.format.duration
	...
	Channel.prototype.moveTo = function(from, to, done) {
		...
		this._writeJSON();
		// check for first position and play
		if(to == 1) {
			if(_.isFunction(this.currentSource.end)) { 
				this.debug(this.id + ' - ' +' Try and stop the current stream ');
				this.currentSource.end(this.playTransition.bind(this));
			} else {
				this.debug(this.id + ' - ' +' The was not an end function. ');
				this.playTransition();
			}
	...
	Channel.prototype.play = function(playThis, done) {
		...
		//this.debug(this.id + '  -  ' +' Is a readable stream? ',  Broadcast.isReadableStream(source.stream));
		...
		new Broadcast.Source.Fluent(source, (err, fluent) => {
			...
			//this.debug(this.id + ' - ' +' source piped to stream with Broadcast.Source.Fluent');
	...
	Channel.prototype.RESTART = function(merge) {
		...
		return this.KILL()	
			...
			.catch(e => {
				this.debug(this.id + ' - ' + ' failed adding channel', 'reject');
			});	
