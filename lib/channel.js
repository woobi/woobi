...
module.exports = function(Broadcast) {
	...
	function Channel(channel, options) { 
		...
		// if we use transitions go ahead and start the stream.
		// hls configs might not work otherwise
		//  sounds like hls might need to just changed s it is run after play.
		...
		async.series([
			...
			(next) => {
				if(options.udpStream) {
					...
					async.forEach(drain, (run, dry) => {
						this.udpStream = new Broadcast.Stream.UDP(_.defaults(run, { channel: channel, port: Broadcast.channelPort++,  stream: this.stream, autoPlay: true, streamOptions: { end: false }}), (err, udp) => {
			...
			(next) => {
				...
				this._loop = this.sources.map(r => r);
			...
			if(options.hls) {
				...
				async.forEach(share, (consume, finish) => {
					 var run = (cb) => {
						this.hls = new Broadcast.Stream.HLS(_.defaults(consume, { source: this.stream }), (err, Source) => {
							if(err) {
	...
	Channel.prototype.play = function(playThis, callback) {
		...
		if(Broadcast.isReadableStream(source.stream)) {
			...
			source.stream.on('end', e => source.end());
