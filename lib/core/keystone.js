...
function key() {
	...
	exports.keystoneServer = function(opts, callback) {
		...
		keystone.init({
			...
			'signin url': opts['signin url'] || false ,
			'signout url': opts['signout url'] || false ,
			'signin redirect': opts['signin redirect'] || false ,
			'signout redirect': opts['signout redirect'] || false ,
		});
		...
		keystone.start({
			onMount: function() {
				...
				Broadcast.libs.mongo = keystone.lists;
			},
			onStart: () => {
