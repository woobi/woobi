...
module.exports = function(Broadcast) {
	...
	return Object.assign( epg( Broadcast ) , {	
		...
		channels(data, callback) {
			...
			_.each(Broadcast.channels, (chan, name) => {
				...
				send.push({
					...
					playing: {
						...
						progress: chan.currentSource.progress || {timemark: '00:00:00', percent:0}
					},
					prev: {
						name: history.name,
			...
			return {
				success: true,
				savedConfigs: Broadcast._options.savedConfigs,
		...
		channel(data, callback) {
			//debug('Send Channel', data);
			if(!_.isObject(data)) {
				data = {};
			}
			let chan = Broadcast.channels[data.channel];
			//debug(chan, Broadcast.channels)
			if(!chan) return;
			let history = chan.history[0] || {}
			let send = {
				id: chan.id,
				mediaPath: Broadcast.mediaPath,
				dbPath: Broadcast.get('dbPath'),
				state: chan.state,
				port: Broadcast.port,
				host: Broadcast.host,
				channelPort: chan.channelPort,
				...
				links: chan.links,
				commands: chan.commands,
				numSources: chan.sources.length,
				playing: {
					name: chan.currentSource.name,
					codecs: chan.currentSource.codecs || {},
					metadata: chan.currentSource.metadata || {},
					duration: chan.currentSource.duration / 60,
					progress: chan.currentSource.progress || { timemark:'0', percent:0}
				},
				prev: {
					name: history.name,
					metadata: history.metadata  || {}
		...
		createChannel(data, callback) {
			debug('Create channel API')
			const pro = data.channel;
			if(pro) {  
				...
				.then(program => {
					...
					callback({
						success: true,
						channel: Broadcast.socketListeners.channel(pro.name)
					})
				})
				.catch(e => {
