...
module.exports = function(app) {
	...
	app.get('/noscript/*', function(req, res) {
		doMatch(req, res, StaticRoutes,	true);
	});
	
	/** FINAL REACT Routes **/
	app.get('*', function(req, res) {
		doMatch(req, res, Routes, false);	
	});
...
function doMatch(req, res, routes, noscript) {
	...
	match({ routes: routes, location: req.url }, function (error, redirectLocation, renderProps) {
		if (error) {
			...
			// console.log(renderProps);
				...
				getInitialData(renderProps, this).then(data2 => {
					// merge the assets with page data
					var initialProps = JSON.stringify(_.assign(IData, data2));
