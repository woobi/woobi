...
var	debug = require('debug')('snowstreams:lib:core:routes');
...
var Routes;
var StaticRoutes;
SystemJS.import('app/routes').then(function(m) {
	Routes = m.default;
...
module.exports = function(app, Broadcast, opts) {
	...
	// add the greeter
	if(Broadcast.keystone) {
		greeter.init({ keystone: Broadcast.keystone, 'allow register': opts.allowRegister }, true);
		greeter.add('/greeter');
		greeter.setField('login', 'password', 'B-password', {
			label: 'password',
			field: 'password',
		...
		if (error) {
			...
			// console.log(renderProps);
				...
				getInitialData(renderProps, Broadcast).then(data2 => {
					...
					res.status(200).send(render( component, storesInitialDataScript, noscript ));
				})
				.catch(console.log);
...
function render(text, script, static) {
	var bodyRender = static ? renderBodyNoScript : renderBody;
	return renderHead() + bodyRender(text, script);
}
...
function renderBodyNoScript(text) {
	return ('<body class="dark-theme default"><div id="body"><div id="react-hot-reload" >' + text + '</div><div className="container"><div id="footer"></div></div></body>');
}

function renderBody(text, script) {
	return ('<body class="dark-theme default"><div id="body"><div id="confirm-modal"></div><div id="react-hot-reload" >' + text + '</div><div className="container"><div id="footer"></div></div><script type="text/javascript" src="https://cdn.jsdelivr.net/clappr/latest/clappr.min.js"></script><script src="https://cdn.jsdelivr.net/clappr.chromecast-plugin/latest/clappr-chromecast-plugin.js"></script><script>' + script + '</script><script src="/js/bundles/material-ui.js" ></script></body>');
}
