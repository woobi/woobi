	 ...
	 */
	 return function (req, res, next) {
		...
		if (_.isUndefined(ipRanges) ) {
			console.log('Allowed IP range is undefined');
			ipRanges = ['127.0.0.1/32'];
		}
		if (!_.isArray(ipRanges) ) {
			console.log('Allowed IP range is not an Array',ipRanges);
			ipRanges = ['127.0.0.1/32'];
		}
		...
		if (!requestAllowed) {
			...
			console.log(util.format(msg2, allowedRanges));
			...
			return res.status(403).redirect("http://www.bethematch.org");
