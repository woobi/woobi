 ...
 */
	...
	function Fluent(opts, callback) {
		...
		function finish ( metadata ) {
			...
			if(opts.progress) {
				if(opts.progress.timemark) this.ffmpeg.seekInput(opts.progress.timemark);
			}	
			if(opts.seekByBytes) {
			...
			//if(opts.progress) {
				this.ffmpeg.on('progress', (progress) => {
					...
					if(Broadcast.channels[this.channel].sources[0])Broadcast.channels[this.channel].sources[0].progress = progress;
