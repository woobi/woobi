...
module.exports = function(Broadcast) {
	...
	return function Program(options, callback) {
		...
		// create Asset
		this.redo = (cmd, cmd2) => {
			if(this.options.reArg) {
				var arg =  this.options.reArg.replace('##CMD##', cmd);
				arg =  this.options.reArg.replace('##CMD2##', cmd2);
				return arg;
			}
		}
		...
		function useSource(opts) {
			...
			Asset.end = (pass, cb) => {
				...
				if(Broadcast.isReadableStream(Asset.stream) && _.isFunction(Asset.stream.end)) {
